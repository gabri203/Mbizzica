<!DOCTYPE html>
<html>
    <head>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
        <title>Mbizzica</title>
    </head>
    <body style="margin: 0; background:#333;">
        <div class="#" style="height:80vh; display:flex; flex-direction:column;justify-content:center; margin:auto; width:300px; border-style:solid; border-width:3px; padding:3em; margin-top:2.2em; background:whitesmoke;">
            <form action="{{ route('password.submit') }}" method="POST">
                <h3>Reset Password</h3>
                @csrf
                {{-- Campi nascosti per il reset con token --}}
                <input type="hidden" name="token" value="{{ request()->token }}">
                <input type="hidden" name="email" value="{{ request()->email }}">{{-- serve per abbinare il token all'utente giusto tramite name="email" --}}
                <div class="mb-3">
                    <label class="form-label">Nuova password</label>
                    <input type="password" name="password" placeholder="Password" class="form-control" required>
                </div>
                <div class="mb-3">
                    <label class="form-label">Conferma nuova password</label>
                    <input type="password" name="password_confirmation" placeholder="Conferma Password" class="form-control" required>
                </div>
                    <button type="submit" class="btn btn-primary">Invio</button>
            </form>

        </div>
    </body>
</html>
